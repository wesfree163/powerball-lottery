...
#     print('Range of white ball number (1 to n, For example 1 to 99)')
#     v_range = int(input('n = '))
...
#     print(f'Enter {power_ball_count} different powerball numbers from 1 to {v_range}, with spaces between')
#     print(f'each number. (For example: {" ".join((str(i) for i in randnums))})')
#     response = input('> ')
...
#         print(f'Please enter {power_ball_count} powerball numbers, separated by spaces.')
#         continue
...
#         print('You must enter {power_ball_count} different numbers.')
#         continue
...
def trial_run_Simulation(w_ball_count, v_range, power_ball_count, p_range, p_winning_ball, trials):
    numbers = []
    while True:

        print('How many white balls to use')
        w_ball_count = w_ball_count
        ...
        # remember to adjust to limit to only 2
        print('How many powerballs to use (For example: 0, 1, or 2)')
        power_ball_count = power_ball_count
        power_array = [0] * power_ball_count
        ...
        print(f'Entering {w_ball_count} different numbers from 1 to {v_range}, with spaces between')
        # print(f'each number. (For example: {" ".join((str(i) for i in randnums))})')
        num = 0
        while num < len(some_numbers):
            some_numbers[num] = random.randint(1, p_range)
            num += 1


        
        print(f'Entering {power_ball_count} different powerball numbers from 1 to {p_range}, with spaces between')
        # print(f'each number. (For example: {" ".join((str(i) for i in randnums))})')
        num = 0
        while num < len(power_array):
            power_array[num] = random.randint(1, v_range)
            num += 1

        # Check that the player entered correct number of things:
        # for i in some_numbers.count:
        #     numbers += i.split(", ")

        if len(some_numbers) != power_ball_count:
            print(f'Please enter {power_ball_count} powerball numbers, separated by spaces.')
            continue
        ...
        # Check that the numbers are between 1 and 69:    
        for i in range(power_ball_count):
            if not (1 <= some_numbers[i] <= v_range):
                while not (1 <= some_numbers[i] <= v_range):
                    print(f'The numbers must all be between 1 and {v_range}.')
                    print(f'Enter {power_ball_count} different numbers from 1 to 69, with spaces between')
                    print('each number. (For example: 5 17 23 42 50)')
                    response = input('> ')
        ...
        # Check that the numbers are unique:
        # (Create a set from number to remove duplicates.)
        if len(set(some_numbers)) != power_ball_count:
            print('You must enter {power_ball_count} different numbers.')
            continue
...
wballRange = random.randint(1, 10)
pballRange = random.randint(1, 5)
pwinBall = random.randint(1, 26)

trial_run_Simulation(5, wballRange, 1, pballRange, pwinBall, 1000)


wballRange = random.randint(1, 15)
pballRange = random.randint(1, 10)
pwinBall = random.randint(1, 26)

trial_run_Simulation(3, wballRange, 1, pballRange, pwinBall, 1000)


# Need to make program accept 2 powerball inputs
wballRange = random.randint(1, 10)
pballRange = random.randint(1, 7)
pwinBall = random.randint(1, 26)

trial_run_Simulation(2, wballRange, 1, pballRange, pwinBall, 1000)
